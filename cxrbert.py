...
class AdapterBertIntermediate(nn.Module):
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        # adapter
        self.scale = 0.5
        self.attn_adapter = Adapter(config.hidden_size, skip_connect=True)
    ...
    def forward(
        ...
        attention_output = self.attn_adapter(attention_output)  # attn_adapter
    ...
    def feed_forward_chunk(self, attention_output):
        ...
        layer_output = self.output(intermediate_output, attention_output) + self.scale * self.mlp_adapter(attention_output)  # MLP_adapter
        return layer_output
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if img_feature is not None:
            embedding_output[embedding_output.shape[0]//2:, :, :] = embedding_output[embedding_output.shape[0]//2:, :, :] + img_feature.unsqueeze(1)  # half for reconstruction
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        if img_feature is not None:
            prediction_scores = self.cls(sequence_output[sequence_output.shape[0]//2:, :, :])  # half for reconstruction
        else:
...
class CXRBertModel(BertForMaskedLM):
    ...
    def __init__(self, config: CXRBertConfig):
        ...
        self.local_projection_head = BertProjectionHead(config)
        self.init_weights()
    ...
    def forward(
        ...
        **kwargs: Any
        ...
        if img_feature is not None:
            last_hidden_state = last_hidden_state[:last_hidden_state.shape[0]//2, :, :]  # half for contrastive
        cls_projected_embedding = (
            self.cls_projection_head(last_hidden_state[:, 0, :]) if output_cls_projected_embedding else None
        )
        local_proected_embedding = (
            self.local_projection_head(last_hidden_state[:, 1:, :]) if output_cls_projected_embedding else None
        )
        if return_dict:
            return CXRBertOutput(
                ...
                local_proected_embedding=local_proected_embedding,
...
if __name__ == "__main__":
    model = CXRBertModel.from_pretrained("./BiomedVLP-CXR-BERT-specialized")
